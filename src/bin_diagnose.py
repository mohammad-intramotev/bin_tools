...
def validate_point_frame(stats, packet):
    ...
    # 2. Check for monotonic hardware timestamp (timestamp_us)
    ...
    # 3. Check for monotonic system timestamp
    ...
    # 4. Check final timestamp offset against expected frame duration
    ...
    # 5. Check for valid reflectivity values
...
def validate_imu_message(stats, msg):
    ...
    # 2. Check for monotonic system timestamp
    ...
    # 3. Check for sane floating point values
...
def analyze_bin_file(bin_file_path):
    ...
    duration_s = (
        (stats['last_timestamp_ms'] - stats['first_timestamp_ms']) / 1000.0
        if stats['first_timestamp_ms'] else 0
    )
    file_size = os.path.getsize(bin_file_path)
    
    # LiDAR-specific calculations
    lidar_frames = stats['total_lidar_frames']
    ...
    frame_refresh_hz = (lidar_frames / duration_s) if duration_s > 0 else 0
    expected_points = duration_s * config.LIVOX_AVIA_POINTS_PER_SECOND
    lidar_capture_rate = (
        (actual_point_rate / config.LIVOX_AVIA_POINTS_PER_SECOND) * 100
        if expected_points > 0 else 0
    )
    
    # IMU-specific calculations
    ...
    imu_capture_rate = (
        (actual_imu_rate / config.LIVOX_AVIA_IMU_RATE_HZ) * 100
        if expected_imu_msgs > 0 else 0
    )
    ...
    print(f"  - Frames Received:         {lidar_frames:,}")
    print(f"  - Total Points Captured:   {stats['total_lidar_points']:,}")
    print(f"  - Frame Refresh Rate:      {frame_refresh_hz:,.2f} Hz")
