...
from common import config, livox_pb2, orientation_pb2
...
def try_combine_imu_data_new(current_livox_imu_data, bag_writer):
    ...
    for d_data in reversed(duro_orientation_buffer):
        ...
        if d_data['logger_timestamp_ms'] <= current_livox_imu_data['logger_timestamp_ms']:
            if (current_livox_imu_data['logger_timestamp_ms'] - d_data['logger_timestamp_ms']) <= config.constants.MAX_ORIENTATION_AGE_MS:
    ...
    if best_duro_data:
        ...
        if imu_msg:
            ...
            bag_writer.write(config.IMU_TOPIC, serialize_message(imu_msg), ros_timestamp_ns)
            return 1
    # else:
...
def create_ros2_bag(bin_file_path, output_bag_dir):
    ...
    topic_metadata_map = {
        config.LIDAR_TOPIC: rosbag2_py.TopicMetadata(name=config.LIDAR_TOPIC, type='sensor_msgs/msg/PointCloud2', serialization_format='cdr'),
        config.IMU_TOPIC: rosbag2_py.TopicMetadata(name=config.IMU_TOPIC, type='sensor_msgs/msg/Imu', serialization_format='cdr')
    }
    bag_writer.create_topic(topic_metadata_map[config.LIDAR_TOPIC])
    bag_writer.create_topic(topic_metadata_map[config.IMU_TOPIC])
    print(f"Registered topics: {config.LIDAR_TOPIC}, {config.IMU_TOPIC} for bag {output_bag_dir}")
        ...
        while True:
            ...
            if channel_name == "avia_points":
                ...
                if ros_msg:
                    ...
                    bag_writer.write(config.LIDAR_TOPIC, serialize_message(ros_msg), ros_timestamp_ns)
